...
//
//  Created by Paul Ruvolo on 10/8/25.
//
...
class PreviewMetalView: MTKView {
    ...
    private func setupTransform(width: Int, height: Int, mirroring: Bool, rotation: Rotation) {
        ...
        print("internalBounds \(internalBounds) \(textureWidth) \(textureHeight)")
    ...
    override func draw(_ rect: CGRect) {
        print("draw \(rect.width)x\(rect.height) \(rect.minX),\(rect.minY)")
        var pixelBuffer: CVPixelBuffer?
        var mirroring = false
        var rotation: Rotation = .rotate0Degrees
        ...
        let textureOut = drawable.texture
        print("rendered")
    }
}
